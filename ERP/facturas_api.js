...
const FacturasAPI = (() => {
  ...
  async function registrarCobro(facturaId, cobro) {
    // RPC atómica — inserta cobro + actualiza estado factura (Fase B)
    const { data, error } = await db.rpc('registrar_cobro_factura', {
      p_factura_id: facturaId,
      p_cobro:      cobro,
    });
    if (error) throw error;
    return data; // { ok: true, cobro_id, estado: 'cobrada' | 'cobro_parcial' }
  }
