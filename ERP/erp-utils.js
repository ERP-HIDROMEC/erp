...
const PERMISOS = {
  admin_general:    ['*'],
  gerencia:         ['*'],
  jefe_operaciones: ['proyectos','remitos','contratos','presupuestos','empleados'],
  administracion:   ['facturas','compras','remitos','contratos','presupuestos','clientes'],
  supervisor:       ['proyectos','remitos','presupuestos'],
};

const MODULOS_FINANCIEROS = ['caja','impuestos'];
const ROLES_FINANCIEROS   = ['admin_general','gerencia'];

let _usuario = null; // cache del usuario actual
...
async function verificarAcceso(modulo) {
  ...
  // Admin y gerencia acceden a todo
  if (['admin_general','gerencia'].includes(rol)) return true;

  // Módulos financieros — solo roles financieros
  if (MODULOS_FINANCIEROS.includes(modulo)) {
    if (!ROLES_FINANCIEROS.includes(rol)) {
  ...
  document.body.innerHTML = `
    <div style="display:flex;align-items:center;justify-content:center;min-height:100vh;background:#0f1117;font-family:'DM Sans',sans-serif">
      <div style="text-align:center;color:#e8eaf0">
        ...
        <a href="index.html" style="background:#3b82f6;color:#fff;padding:10px 20px;border-radius:8px;text-decoration:none">Volver al dashboard</a>
      </div>
    </div>`;
}
...
// Auto-init al cargar
document.addEventListener('DOMContentLoaded', () => initBuscadorGlobal());
